...
int main() {
    ...
    DataLoader loader("vol_surface_sample.csv");
    ...
    std::cout << "    Entre K=100 (0.22) et K=105 (0.225)" << std::endl;
    double vol_interp_strike = surface.getVol(0.25, 102.5);
    std::cout << "    Valeur obtenue     : " << vol_interp_strike << std::endl;
    std::cout << "    Valeur attendue    : 0.2225" << std::endl;
    assert(almostEqual(vol_interp_strike, 0.2225, 1e-10));
    ...
    std::cout << "    Entre T=0.25 (0.22) et T=0.5 (0.215)" << std::endl;
    double vol_interp_maturity = surface.getVol(0.375, 100.0);
    std::cout << "    Valeur obtenue     : " << vol_interp_maturity << std::endl;
    std::cout << "    Valeur attendue    : 0.2175" << std::endl;
    assert(almostEqual(vol_interp_maturity, 0.2175, 1e-10));
    ...
    std::cout << "    Coins du rectangle :" << std::endl;
    std::cout << "      (0.25,100) = 0.22" << std::endl;
    std::cout << "      (0.25,105) = 0.225" << std::endl;
    std::cout << "      (0.5,100)  = 0.215" << std::endl;
    std::cout << "      (0.5,105)  = 0.22" << std::endl;
    double vol_bilinear = surface.getVol(0.375, 102.5);
    std::cout << "    Valeur obtenue     : " << vol_bilinear << std::endl;
    std::cout << "    Valeur attendue    : 0.22" << std::endl;
    assert(almostEqual(vol_bilinear, 0.22, 1e-10));
